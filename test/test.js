...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( range.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the range of a one-dimensional ndarray', function test( t ) {
	...
	v = range( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 9.0, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = range( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = range( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = range( [ vector( x, 1, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = range( [ vector( x, 0, 1, 0 ) ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	t.end();
});

tape( 'if provided an ndarray containing a single element, the function returns `0`', function test( t ) {
	var x;
	var v;
